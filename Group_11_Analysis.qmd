...
@fig-cooks-distance presents Cook’s Distance, which assesses the influence of individual observations on the Stepwise Logistic Regression model estimates. The plot shows that most observations have low Cook’s Distance values, with no apparent extreme outliers or highly influential points. This suggests that the model’s fit is not significantly affected by any single observation, indicating good overall robustness.
```{r}
#| echo: false
