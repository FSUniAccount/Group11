...
Based on this table, we can conclude the Stepwise model, with an AIC of 30078.14 is a better fit than the full model, with AIC of 30080.98. The Stepwise function has retained variables of AgeGroup, Sex, Employment and Veg, so the only removed variable is fruit.
...
The odds ratio assesses how the odds of the dependent variable(Obese) change with a 1 unit change in the independent variable(AgeGroup, Sex, Employment, Veg). 
The lower and Upper 95% Confidence Interval indicates the range in which the Odds Ratio will lie for each variable. 
The P-Value tests the null hypothesis that the Odds Ratio is equal to 1. For all variables, the p-value < 0.05, meaning it is statistically significant. 
...
From this plot, we can see that the Odds Ratio increases as AgeGroup increases.
Permanently unemployed have the highest Odds Ratio whereas the other employment status have very similar Odds Ratios around 1, apart from Full-time Education which has lowest Odds ratio.
SexMale has an Odds ratio close to 1 and VegYes has an Odds Ratio of less than 1. 
