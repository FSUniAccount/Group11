...
#| tbl-cap: AIC comparison between full and stepwise models with retained variables.    
...
Based on this table, we can conclude the Stepwise model, with an AIC of 30078.14 is a better fit than the full model, with AIC of 30080.98. The Stepwise function has retained variables of AgeGroup, Sex, Employment and Veg, so the only removed variables are fruit and year.

The full model regression will look like this:

$y_i=\beta_0+\beta_1\cdot{Year}+\beta_2\cdot{AgeGroup}+\beta_3\cdot{Sex}+\beta_4\cdot{Employment}+\beta_5\cdot{Fruit}+\beta_6\cdot{Veg}$

where

-    $y_i$ is the Odds of being obese

-   $\beta_0$ is the intercept of the model

-   $\beta_1, . .., \beta_6$ are the coefficients of the corresponding variables

The Stepwise model looks like this:

$y_i=\beta_0+\beta_2\cdot{AgeGroup}+\beta_3\cdot{Sex}+\beta_4\cdot{Employment}+\beta_6\cdot{Veg}$
...
-   $\beta_1,\beta_2,\beta_3,\beta_4 and \beta_5$ are the coefficients of the corresponding variables
...
#| fig-cap: Forest plot of Odds Ratios from the stepwise model with 95% confidence intervals.
#| fig-align: center
#| fig-width: 6
#| fig-height: 4
#| theme: minimal

ggplot(odds_ratios_df, aes(x = reorder(Variable, OR), y = OR, ymin = LowerCI, ymax = UpperCI)) +
  geom_pointrange(color = "steelblue", size = 1) +  
  geom_hline(yintercept = 1, linetype = "dashed", color = "gray", linewidth = 1) +  
  coord_flip() +  
  labs(title = "Odds Ratios for Obesity Prevalence",
...
From this plot, we can see that the Odds Ratio increases as AgeGroup increases. Permanently unemployed have the highest Odds Ratio whereas the other employment status have very similar Odds Ratios around 1, apart from Full-time Education which has lowest Odds ratio. SexMale has an Odds ratio close to 1 and VegYes has an Odds Ratio of less than 1.
...
check_overdispersion(stepwise_model)
#No Overdispersion detected means that the standard errors of the model's coefficients have not been underestimated
#Overdispersion can lead to type I errors.

```

No Overdispersion means that the standard errors of the model's coefficients have not been underestimated. Overdispersion can lead to type I errors.

### Check Outliers & Cooks Distance
...
check_normality(residuals(stepwise_model, type = "deviance"))
```

This checks if the deviance residuals follow a normal distribution. Because p \< 0.001 there is strong evidence against the null hypothesis of normality. This is fine; in logistic regression, the normality of deviance residuals is not a primary assumption.
...
qqnorm(residuals(stepwise_model, type = "deviance"), main = "Q-Q Plot of Deviance Residuals")
qqline(residuals(stepwise_model, type = "deviance"))
```
...
hoslem.test(data$Obese, predicted_probabilities)

```

The P value (0.1315) is much greater than 0.05, and therefore there is not enough evidence to reject the null hypothesis that the model fits the data: ie. that our model reasonably fits the data.
